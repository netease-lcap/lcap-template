...
describe('ListDistinctBy', () => {
  test('兼容性测试，去重原生数字', () => {
    {
      const ansAsc = [-100, 0, 100];
      const testArr1 = [-100, 0, 100, 0, 0, 100, -100];
      expect(ansAsc).toEqual(utils.ListDistinctBy(testArr1, [(item) => item]));
    }
  });
  ...
  test('去重复杂对象', () => {
...

    {
      ...
      expect(ansAsc).toEqual(utils.ListDistinctBy(testArr1, [(item) => item.gender]));
    }

    {
      const ansAsc = [obj1, obj2];
      const testArr1 = [obj1, obj2, obj3];
      expect(ansAsc).toEqual(utils.ListDistinctBy(testArr1, [(item) => item.age]));
    }
  });
});
