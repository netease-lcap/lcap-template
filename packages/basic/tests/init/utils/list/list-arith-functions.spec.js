...
describe('List arithmetic (aggregation) functions', () => {
  ...
  test('测试非正常输入', () => {
    fns.forEach((fn) => {
      expect(fn.call(utils, undefined)).toBeNull;
    });

    fns.forEach((fn) => {
      expect(fn.call(utils, null)).toBeNull;
    });

    fns.forEach((fn) => {
      expect(fn.call(utils, [])).toBeNull;
    });

    fns.forEach((fn) => {
      expect(fn.call(utils, [undefined, null, null])).toBeNull;
    });
  });
