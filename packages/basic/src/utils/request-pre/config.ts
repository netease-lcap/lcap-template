...
export default {
  async pre(config: ApiSchemaConfig, configMap: apiConfig, initData: any, ctx: ApiSchema): Promise<any> {
    if (config) {
      return getConfig(config, configMap, 50).reduce(
        (pre: Promise<any>, configHandler: ConfigHandler): Promise<any> => {
          return pre.then(
            (prevData: any): void => {
  ...
  async post(config: ApiSchemaConfig, configMap: apiConfig, initData: requestReturn, ctx: ApiSchema): Promise<any> {
    if (config) {
      return getConfig(config, configMap, 50).reduce(
        (pre: requestReturn, configHandler: ConfigHandler): requestReturn => {
          return pre.then(
            (prevData: any): requestReturn => {
              if (configHandler.resolve) {
