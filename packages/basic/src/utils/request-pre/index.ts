...
class Service {
  public constructor(requester: requester) {
    ...
    this.middlewareList = [requestType];
  ...
  public generator<T extends ApiSchemaList, U extends APIDynamic>(
    apiSchemaList: T,
    dynamicServices?: U,
    serviceConfig?: ServiceConfig,
  ): API<T> & U & APIDynamic {
    ...
    if (typeof Proxy === 'undefined') {
      ...
      return new Proxy(services, {
        ...
        set(target, propertyKey: string, value, receiver: object): boolean {
          if (propertyKey in apiSchemaList) {
            if (process.env.NODE_ENV === 'development') console.error(`can not set property ${propertyKey}`);
            return false;
          } else {
